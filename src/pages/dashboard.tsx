...
import { SummaryListPage as ShowCaseList } from 'pages/summaries';
...
function DashboardPage() {
  ...
  const summaryStat = useSWR<PaginatedInterface<SummaryInterface>>(
    () => `/summaries`,
    () => getSummaries({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    ...
    {
      title: 'Summaries',
      result: summaryStat?.data?.totalCount,
      isLoading: summaryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`summaries`} passHref legacyBehavior>
                  <Button
